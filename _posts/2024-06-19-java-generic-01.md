...
- 단계별 과정을 거치면서 제네릭을 이해하고 제네릭이 왜 필요한지에 대해서 알아보자.

<br>

## Step 01. 

#### 숫자를 보관하고 꺼내는 기능을 가진 박스가 있다.
...
● 간단하게 우리가 원하는 형태의 값을 넣고 꺼내는 기능을 가진 박스를 만들 수 있다.   
● 만약 Double, Boolean 등을 담을 박스가 필요하면 위와 같이 어렵지 않게 만들 수 있다.  
● 그런데 만약 담아야 할 형태의 종류가 수십, 수백가지라면 수십, 수백개의 박스를 만들어야만 한다. 생각만 해도 귀찮은 작업이다.  
● 코드를 재사용할 수 있다면 이 문제를 해결할 수 있지 않을까?

<br><br><br>

## Step 02. 다형성을 통한 중복 제거

- Object는 모든 타입의 부모이다. Object를 사용해 다향성을 활용하여 중복 문제를 해결해 보자.
...
● 이전과 다르게 `ObjectBox`를 통해 Integer 타입 박스와 String 타입 박스로 사용이 가능하다.  
● 말했다시피 Object는 모든 타입의 부모이기 때문이다.  
...
● `ObjectBox`를 사용하여 `Integer` 타입만 담도록 약속을 하고 박스를 만들었다.  
...
#### 하지만 우리는 보통 위 과정을 아래와 같이 다운 캐스팅 처리를 한다.  

```java
Integer integer = (Integer) integerBox.get();
```

● `IntegerBox`에는 `Integer` 타입의 값만 존재한다는 약속이 되어 있기 때문이다.  
● 하지만 누군가가 잘못된 타입의 인수를 전달한 경우(IntegerBox에 String 타입의 hello 전달)   
● `String`은 `Integer`로 캐스팅 되지 않기 때문에 캐스팅 오류(ClassCastException)가 발생한다.

● Object를 통해 `다형성`을 활용한 덕분에 코드를 재사용할 수 있게 되어 중복 문제를 해결할 수 있었다.  
...
● 제네릭 클래스는 타입을 미리 지정하지 않는다. 타입은 제네릭 클래스를 사용할 시점에 정해진다.  
● 최종적으로 클래스명 오른쪽에 `<T>` 이런 형태로 사용된다. `GenericBox<T>`  
...
● GenericBox 클래스를 생성하는 시점에 `<>` 안에 타입을 지정해 준다. `new Generic<Integer>()`  
● 타입을 넣은 제네릭 클래스 생성하면 그때 제네릭 클래스의 타입은 정해진다.               
...
● 그 결과 `set()` 메서드를 통해 Integer 타입의 값만 받을 수 있다.  
● 다른 타입을 인수로 전달하게 되면 컴파일 오류가 발생하게 된다.  
● `get()` 메서드를 통해서는 Integer 타입의 값을 반환하게 된다.  
● 그래서 Object일 때는 다운 캐스팅을 해줘야 했지만 제네릭 클래스는 캐스팅을 해주지 않아도 된다.  

● 참고로 실제 코드가 위 처럼 `GenericBox<Integer>`로 변하는 것이 아니다.  
● 자바 컴파일러가 우리가 입력한 타입을 기반으로 코드를 컴파일 하는 시점에 타입 정보를 반영한다.  
● 따라서 타입이 다른 경우 컴파일 오류가 발생하는 것이다.

<br>

#### 다른 타입 인수 전달 시 컴파일 오류 발생
...
● `GenericBox`를 `Integer` 타입으로 지정한 경우 String 타입 인수를 전달하면 `컴파일 오류`가 발생한다.  
● 컴파일 시점에 오류를 찾아낼 수 있도록 작성하는게 좋은 코드의 조건 중 하나이다.  
